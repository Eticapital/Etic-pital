...
class MimeIconParser {
    ...
    protected $type_extensions = [
        'word' => 'doc,dot,wbk,docx,docm,dotx,dotm,docb,pages,odt,rtf,tex,wks,wps,wpd',
        'spreadsheet' => 'xls,xlt,xlm,xlsx,xlsm,xltx,xltm,xlsb,xla,xlam,xll,xlw,numbers,csv,ods',
        'presentation' => 'ppt,pot,pps,pptx,pptm,potx,potm,ppam,ppsx,ppsm,sldx,sldm,key,odp',
        'pdf' => 'pdf',
        'text' => 'txt',
        'code' => 'html,htm,xml,xhtml,js,php,asp,asm,aspx,inc,vue,jad,java,json,jsp,lib,o,rc,script,src,vbs,xcodeproj,xsd,xsl,xslt,swift,sh,c,class,cpp,h,vb,cer,cfm,cgi,pl,pt,rss,sql',
    ...
    /**
     * Obtengo todas las extensiones que actualmente se mapean a un
     * tipo de archivo, util para pruebas
     *
     * @return array
     */
    public function extensionsWithIcons()
    {
        return collect($this->type_extensions)->map(function ($extensions, $type) {
            $extensions = collect(explode(',', $extensions))->map(function ($e) {
                return trim($e);
            });
            return $extensions;
        })->flatten()->toArray();
    ...
    {
        $mimes = collect($this->extension_mimes)->get($extension);
        if (count($mimes)) {
            return collect($mimes)->random();
        }

        return null;
    }

    public function getIconByMime($mime)
    {
        $mimes = collect($this->extension_mimes)->filter(function ($mimes, $extension) use ($mime) {
            return collect($mimes)->contains(function ($m) use ($mime) {
                return strtoupper(trim($m)) == strtoupper(trim($mime));
            });
        });

        $extension = $mimes->keys()->first();

        return $extension ? $this->getIconByExtension($extension) : $this->type_icon['default'];
    }

    public function getIconByExtension($extension)
    {
        $type = collect($this->type_extensions)->flip()->first(function ($type, $extensions) use ($extension) {
            return collect(explode(',', $extensions))->contains(function ($e) use ($extension) {
                return strtoupper(trim($e)) == strtoupper(trim($extension));
            });
        });
