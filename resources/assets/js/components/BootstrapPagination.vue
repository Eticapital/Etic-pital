    ...
    <nav aria-label="...">
        <ul class="pagination justify-content-center" v-if="pagination.last_page > 0" :class="sizeClass">
            <li v-if="showPrevious()" :class="{ 'disabled' : pagination.current_page <= 1, 'page-item' : true }">
                <span class="page-link" v-if="pagination.current_page <= 1">
                    <span aria-hidden="true" v-html="config.previousText"></span>
                </span>

                <a class="page-link" href="#" v-if="pagination.current_page > 1 " :aria-label="config.ariaPrevioius" @click.prevent="changePage(pagination.current_page - 1)">
                    <span aria-hidden="true" v-html="config.previousText"></span>
                </a>
            </li>
            ...
            <li v-if="showNext()" :class="{ 'disabled' : pagination.current_page === pagination.last_page || pagination.last_page === 0, 'page-item' : true }">
                <span v-if="pagination.current_page === pagination.last_page || pagination.last_page === 0">
                    <span class="page-link" aria-hidden="true" v-html="config.nextText"></span>
                </span>
