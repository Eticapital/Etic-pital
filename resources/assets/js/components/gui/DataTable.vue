<template>
  ...
  <table :class="['vuetable', css.tableClass]">
    ...
    <tbody v-if="busy" class="data-table-data-loading">
      <tr>
        <td ref="dataTableLoading" :colspan="tableFields.length">
          <span class="data-table-loading-message">
            <i class="spinner icon-spinner"></i> Cargando...
          </span>
        </td>
      </tr>
    </tbody>
    <tbody v-else-if="!tableData.length" class="data-table-data-loading">
      <tr>
        <td ref="dataTableEmpty" :colspan="tableFields.length">
          <span class="data-table-no-results">
            <p v-if="query">
                <i class="icon-search"></i>
                <span v-text="'No se encontraron resultados con tu criterio de búsqueda'"></span>
            </p>
            <p v-if="query"><button class="btn btn-primary" @click.prevent="clearSearch">Borrar criterio de búsqueda</button></p>
            <p v-else>
              <span v-text="'Sin datos que mostrar'"></span>
            </p>
          </span>
        </td>
      </tr>
    </tbody>
    <tbody ref="dataTableLoaded" v-cloak v-else class="vuetable-body">
...
export default {
  ...
  watch: {
    busy: function (value) {
      if (!value) {
        this.$refs.dataTableLoading.style.height = null;
        return false;
      }
      var dataHeight = this.$refs.dataTableLoaded ? this.$refs.dataTableLoaded.offsetHeight : 0;
      if (dataHeight) {
        this.$refs.dataTableLoaded.style.height = null;
        Vue.nextTick(() => {
          this.$refs.dataTableLoading.style.height = dataHeight + 'px';
        });
      }
    },
